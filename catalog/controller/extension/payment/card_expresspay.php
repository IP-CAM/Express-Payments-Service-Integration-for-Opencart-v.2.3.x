...
class ControllerExtensionPaymentCardExpressPay extends Controller {
	public function index() {
		...
		$data['redirect'] = $this->url->link('extension/payment/card_expresspay/send');
		$data['text_loading'] = $this->language->get('text_loading');

		return $this->load->view('extension/payment/card_expresspay.tpl', $data);
	}
	...
	public function send() {
        ...
        $request_params = array(
            ...
            "Currency" => $currency,
            "Info" => $invoice_info,
            "ReturnUrl" => $this->url->link('extension/payment/card_expresspay/success'),
            "FailUrl" => $this->url->link('extension/payment/card_expresspay/fail'),
        ...
        try {
    		...
    		$this->response->redirect($this->url->link('extension/payment/card_expresspay/fail'));
    	}
		...
		$this->log_info("ERROR - ", json_encode($response));
		
        if(isset($response['Error']) || !isset($response['CardInvoiceNo'])){
            $this->log_error('send', 'Get error; ORDER ID - ' . $this->session->data['order_id'] . '; RESPONSE - ' .json_encode($response));
        ...
        if($is_use_signature){
            $requestParams = array(
			 ...
			 $this->log_info('send', 'secret_word - ' . $secret_word);
        ...
        if(isset($response['Error'])  || !isset($response['FormUrl'])){
            $this->log_error('send', 'Get error; ORDER ID - ' . $this->session->data['order_id'] . '; RESPONSE - ' . json_encode($response) . '; ERROR - ������ ��������������� �� ����� ������');
	...
	public function success() {
		...
		$this->response->setOutput($this->load->view('extension/payment/card_expresspay_success.tpl', $data));
	}
	...
	public function fail() {
		...
		$this->response->setOutput($this->load->view('extension/payment/card_expresspay_failure.tpl', $data));
	}
	...
	public function notify() {
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			$secret_word = $this->config->get('card_expresspay_secret_key_notify');
			$is_use_signature = ( $this->config->get('card_expresspay_sign_notify') == 'on' ) ? true : false;
        ...
        if(isset($data->CmdType)) {
        	switch ($data->CmdType) {
        		case '1':
        			...
        			break;
        		case '2':
        			$this->model_checkout_order->addOrderHistory($data->AccountNo, $this->config->get('card_expresspay_cancel_status_id'));
    ...
    private function compute_signature_get_payment_form($request_params, $secret_word) {
        ...
        $result = $this->config->get('card_expresspay_token');
        foreach ($api_method as $item)
			$result .= ( isset($normalized_params[$item]) ) ? $normalized_params[$item] : '';

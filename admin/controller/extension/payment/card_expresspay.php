...
class ControllerExtensionPaymentCardExpressPay extends Controller {
    ...
    public function index() {
		...
		$data['handler_url']  	  = str_replace('/admin', '', HTTPS_SERVER . 'index.php?route=extension/payment/card_expresspay/notify');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/card_expresspay', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/payment/card_expresspay', 'token=' . $this->session->data['token'], true);
